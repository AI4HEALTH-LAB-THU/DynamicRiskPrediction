...
MODEL_DIR = "models/bench_dynamic/catboost/trained_models/251209_final"
RESULT_DIR = "result/bench_dynamic/catboost/prediction_results/251209_final"
...
def optuna_tune_catboost(
    ...
    """使用Optuna进行CatBoost超参数调优（基于county-based划分）"""
...
def train_evaluate_model(
    ...
    for fold_idx, fold_counties in enumerate(outer_folds):
        ...
        # Boruta特征选择（在外层训练集上）
        selected_features = run_boruta(outer_train_df, features, label_col)
        
        if len(selected_features) < 6:
            print("  Boruta选出的特征过少，退回使用原始特征集")
            selected_features = list(features)

        # 内层Optuna调参（使用CatBoost调参函数）
        best_params = optuna_tune_catboost(
            outer_train_df,
            selected_features,
            label_col,
            n_trials=20,
        ...
        # 在外层训练集上训练最终模型（使用CatBoost）
        model_outer = CatBoostClassifier(
            loss_function="Logloss",
            task_type="CPU",
            thread_count=48,
            verbose=0,
            random_seed=SEED + fold_idx,** best_params
        )
    ...
    # 最终模型超参数调优（使用CatBoost调参）
    best_params_final = optuna_tune_catboost(
        train_df,
...
def main():
    print("===== 开始 20 种疾病的 CatBoost 嵌套交叉验证建模 =====")  # 更新模型名称
