...
MODEL_DIR = "models/bench_dynamic/logistic/trained_models/251209_final"
RESULT_DIR = "result/bench_dynamic/logistic/prediction_results/251209_final"
...
def optuna_tune_logistic(
    ...
    """使用Optuna进行Logistic回归超参数调优（基于county-based划分）"""
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        # 定义超参数搜索空间
        params = {
            "C": trial.suggest_float("C", 0.001, 5, log=True),
            "penalty": "l2",
            "solver": "lbfgs"
        }
...
def train_evaluate_model(
    ...
    for fold_idx, fold_counties in enumerate(outer_folds):
        ...
        # 内层Optuna调参（在外层训练集上）
        best_params = optuna_tune_logistic(
            outer_train_df,
            selected_features,
            label_col,
            n_trials=20,
        ...
        X_outer_train_scaled = scaler.fit_transform(X_outer_train_sel)
        ...
        model_outer.fit(X_outer_train_scaled, outer_train_df[label_col].values)

        # 外层测试集预测
        X_outer_test_sel = outer_test_df[selected_features].apply(pd.to_numeric, errors="coerce").fillna(0)
        X_outer_test_scaled = scaler.transform(X_outer_test_sel)
        y_outer_test = outer_test_df[label_col].values
        y_pred_raw = model_outer.predict_proba(X_outer_test_scaled)[:, 1]
    ...
    # 最终模型超参数调优
    best_params_final = optuna_tune_logistic(
        train_df,
    ...
    final_model = LogisticRegression(
        max_iter=2000,
        random_state=SEED,** best_params_final
    )

    final_model.fit(X_train_final_scaled, y_train_full)
    
    # 测试集标准化（使用训练集的scaler）
    X_test_final = test_df[final_selected_features].apply(pd.to_numeric, errors="coerce").fillna(0)
    X_test_final_scaled = scaler_final.transform(X_test_final)
    y_test_pred_raw = final_model.predict_proba(X_test_final_scaled)[:, 1]
    ...
    scaler_path = os.path.join(MODEL_DIR, f"{outcome}_{model_type}_scaler.pkl")
...
def main():
    print("===== 开始 20 种疾病的 Logistic 回归嵌套交叉验证建模 =====")
