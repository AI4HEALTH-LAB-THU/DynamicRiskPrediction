...
MODEL_DIR = "models/bench_dynamic/lgb/trained_models/251209_final"  # 修改模型目录
RESULT_DIR = "result/bench_dynamic/lgb/prediction_results/251209_final"  # 修改结果目录
...
def optuna_tune_lgb(
    ...
    """使用Optuna进行LightGBM超参数调优（基于county-based划分）"""
...
def train_evaluate_model(
    ...
    for fold_idx, fold_counties in enumerate(outer_folds):
        ...
        # 内层Optuna调参（调用LightGBM调参函数）
        best_params = optuna_tune_lgb(
            outer_train_df,
            selected_features,
            label_col,
            n_trials=20,
        ...
        # 在外层训练集上训练最终模型（替换为LightGBM）
        model_outer = lgb.LGBMClassifier(
            objective="binary",
            n_jobs=48,
    ...
    # 全训练集上再做一次Boruta特征选择
    final_selected_features = run_boruta(train_df, features, label_col)
    
    if len(final_selected_features) < 6:
        print("  Boruta选出的特征过少，退回使用原始特征集")
        final_selected_features = list(features)

    # 最终模型超参数调优（使用LightGBM调参）
    best_params_final = optuna_tune_lgb(
        train_df,
...
def main():
    print("===== 开始 20 种疾病的 LightGBM 嵌套交叉验证建模 =====")  # 修改模型名称
